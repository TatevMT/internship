...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query("SELECT u FROM UserEntity u LEFT JOIN u.listOfUserCommunities WHERE " +
            ":status IS NULL OR u.status = :status")
    List<UserEntity> findAllUsers(String status);

    @Query("SELECT new com.internship.service.model.UserWrapper(u) FROM UserEntity u WHERE" +
            ":status IS NULL OR u.status = :status")
    List<UserWrapper> findAllUsersWithWrapper(String status);
